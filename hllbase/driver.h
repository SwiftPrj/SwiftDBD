...
namespace mem {
	...
	bool find_driver() {
		driver_handle = CreateFileW((L"\\\\.\\\swiftproject"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	...
	void read_physical(PVOID address, PVOID buffer, DWORD size) {
		...
		DeviceIoControl(driver_handle, code_rw, &arguments, sizeof(arguments), &arguments, sizeof(arguments), NULL, NULL);
	}
